...
impl RPCServer {
  ...
  pub fn scan_for_processes(&mut self) {
    ...
    match process_server.scan_and_publish() {
      Ok(_) => {}
      Err(err) => {
  ...
  /**
   * Incrementally process a single process start/exec event.
   */
  pub fn process_started(&mut self, pid: u64, path: String, arguments: Option<String>) {
    if self.connectors.is_none() {
      log!("[RPC Server] Cannot process start event, connectors are not initialized");
      return;
    }
    ...
    if let Err(err) = process_server.process_started(pid, path, arguments) {
      log!("[RPC Server] Error while processing start event: {}", err);
    }
  }

  /**
   * Incrementally process a single process exit event.
   */
  pub fn process_exited(&mut self, pid: u64) {
    if self.connectors.is_none() {
      log!("[RPC Server] Cannot process exit event, connectors are not initialized");
      return;
    }
