...
pub struct ClientConnector {
  ...
  process_payload: Arc<Mutex<Option<String>>>,
...
impl ClientConnector {
  pub fn new(
    ...
    ClientConnector {
      ...
      last_pid: None,
      active_socket: None,
      process_payload: Arc::new(Mutex::new(None)),
  ...
  pub fn start(&mut self) {
    ...
    let process_payload = self.process_payload.clone();
      ...
      loop {
        match server.poll_event() {
          Event::Connect(client_id, responder) => {
            ...
            // Re-read the cached process payload after insertion. This closes the
            // race where a process event arrives between the connection response
            // and adding the client to the broadcast set.
            if let Some(payload) = process_payload.lock().unwrap().clone() {
    ...
    std::thread::spawn(move || {
      while let Ok(proc_event) = proc_event_rec.recv() {
        ...
        if proc_activity.id == "null" {
          ...
          proc_clone.last_pid = None;
          *proc_clone.process_payload.lock().unwrap() = None;
        ...
        // Clear a different activity before publishing its replacement.
        ...
        match serde_json::to_string(&payload_struct) {
          Ok(payload) => {
            *proc_clone.process_payload.lock().unwrap() = Some(payload.clone());
            proc_clone.send_data(payload);
          }
          Err(err) => log!(
