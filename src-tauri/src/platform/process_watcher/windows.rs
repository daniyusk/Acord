  ...
  F: Fn() + Send + Sync + 'static,
{
  if WATCHER_RUNNING.swap(true, Ordering::Relaxed) {
    log!("[Windows] Process watcher is already running");
    return;
  }
  ...
  thread::spawn(move || {
    log!("[Windows] Starting lightweight process watcher...");
    ...
    while WATCHER_RUNNING.load(Ordering::Relaxed) {
      ...
      if current_pids != last_pids {
        ...
        log!("[Windows] Process change detected!");
        on_change_clone();
      }
    }

    log!("[Windows] Process watcher stopped.");
  });
}
