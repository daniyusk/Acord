  ...
  F: Fn() + Send + Sync + 'static,
{
  if WATCHER_RUNNING.swap(true, Ordering::Relaxed) {
    log!("[Linux] Process watcher is already running");
    return;
  }
  ...
  thread::spawn(move || {
    log!("[Linux] Starting lightweight process watcher...");
    ...
    while WATCHER_RUNNING.load(Ordering::Relaxed) {
      ...
      if current_pids != last_pids {
        ...
        log!("[Linux] Process change detected!");
        on_change_clone();
      }
    }

    log!("[Linux] Process watcher stopped.");
  });
}
