...
pub fn start_rpc_server(win: tauri::WebviewWindow) {
  ...
  let detectable = match tauri::async_runtime::block_on(async {
    reqwest::get("https://discord.com/api/v9/applications/detectable").await
  }) {
    Ok(resp) => match tauri::async_runtime::block_on(async { resp.text().await }) {
      Ok(text) => text,
      Err(e) => {
